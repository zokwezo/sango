...
func (h *HMM) Train(data []TrainingInstance) {
	...
	// Count unique (state, token) type configurations for Kneser-Ney denominator
	for _, tokens := range h.Emissions {
		h.UniquePairs += int64(len(tokens))
	}
}
